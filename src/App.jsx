...
function StreakView({ currentUser }) {
  ...
  function prevMonth() {
    // No lower limit — go as far back as needed
    ...
    <div>
      ...
      <Card style={{ marginBottom:16 }}>
            ...
            return (
              <motion.button key={i}
                ...
                style={{
                  ...
                  background: isSelected ? T.blue : bgMap[dc],
                  border: isSelected ? `2px solid ${T.blue}` : isToday ? `2px solid ${T.blue}` : `1px solid ${colorMap[dc]}`,
                  cursor: "pointer", minHeight:32,
                  boxShadow: isSelected ? `0 0 12px ${T.blue}55` : "none",
                  outline:"none",
                }}>
                <span style={{ fontSize:11, fontWeight: isToday||isSelected?800:isTrained?700:400,
                  color: isSelected?"#fff":isTrained?T.blue:dc==="skipped"?T.danger:T.muted }}>
                  {d}
                </span>
        ...
        <div style={{ display:"flex", gap:12, marginTop:12, flexWrap:"wrap" }}>
          {[
            ...
            { color:T.border, bg:T.cardHi,         label:"Rest (Sun)" },
          ].map(l => (
          ...
          <div style={{ fontSize:10, color:T.muted, marginLeft:"auto", fontStyle:"italic" }}>Tap any date for details</div>
        </div>
      </Card>
      ...
      <AnimatePresence>
        {selectedDate && (() => {
          ...
          const dayPRs = Object.entries(S.get(`wt_prs_${currentUser.id}`,{}))
            .filter(([,pr]) => pr.maxWeightDate === selectedDate || pr.maxVolumeDate === selectedDate);
          const dc = dayColor(selectedDate);
          const dayVol = Math.round(daySessions.reduce((t,s)=>t+s.exercises.reduce((tt,e)=>tt+totalVol(e.sets),0),0));
          ...
          return (
            <motion.div
              ...
              <Card style={{
                border: `1px solid ${dc==="trained"?T.blue:dc==="skipped"?T.danger:T.border}`,
                background: dc==="trained"?`${T.blue}09`:T.card,
                overflow:"hidden"
              }}>
                <div style={{ height:3, background:`linear-gradient(90deg,${dc==="trained"?T.blue:dc==="skipped"?T.danger:T.border},${T.yellow})` }}/>
                <div style={{ padding:"14px 16px" }}>
                  {/* Date header */}
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
                    <div>
                      ...
                      <div style={{ fontSize:11, color:T.muted, marginTop:2 }}>
                        {dc==="trained" ? `${daySessions.length} session${daySessions.length!==1?"s":""} · ${dayVol}kg volume` :
                         dc==="skipped" ? "No session logged" :
                         dc==="rest"    ? "Rest day (Sunday)" : "Future date"}
                      </div>
                    </div>
                    <motion.button whileTap={{scale:0.9}} onClick={()=>setSelectedDate(null)}
                      style={{ background:"none", border:"none", cursor:"pointer", padding:4 }}>
                      <Icon name="close" size={16} color={T.muted}/>
                    </motion.button>
                  </div>
                  ...
                  {daySessions.length > 0 ? daySessions.map(s => {
                    const m = SCHEDULE[s.day] || { color:T.blue };
                    return (
                      <div key={s.id}>
                        {s.exercises.map((ex,ei) => (
                          <div key={ex.name} style={{
                            paddingTop:ei>0?10:0, marginTop:ei>0?10:0,
                            borderTop:ei>0?`1px solid ${T.border}`:"none"
                          }}>
                            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
                              <div style={{ fontSize:13, fontWeight:700, color:T.text }}>{ex.name}</div>
                              <div style={{ display:"flex", gap:10 }}>
                                <div style={{ textAlign:"right" }}>
                                  <div style={{ fontSize:12, fontWeight:800, color:T.yellow }}>{Math.max(...ex.sets.map(s=>parseFloat(s.weight)||0))}kg</div>
                                  <div style={{ fontSize:9, color:T.muted }}>TOP</div>
                                </div>
                                <div style={{ textAlign:"right" }}>
                                  <div style={{ fontSize:12, fontWeight:800, color:T.blue }}>{Math.round(totalVol(ex.sets))}kg</div>
                                  <div style={{ fontSize:9, color:T.muted }}>VOL</div>
                                </div>
                              </div>
                            </div>
                            <div style={{ display:"flex", flexWrap:"wrap", gap:4 }}>
                              {ex.sets.map((st,si) => (
                                <span key={si} style={{ fontSize:11, color:T.muted,
                        ...
                        {s.notes && (
                          <div style={{ fontSize:11, color:T.purple, marginTop:10, fontStyle:"italic",
                            padding:"8px 12px", background:`${T.purple}11`, borderRadius:8 }}>
                            "{s.notes}"
                          </div>
                        )}
                      </div>
                    ...
                    <div style={{ textAlign:"center", padding:"20px 0", color:T.muted, fontSize:13 }}>
                      {dc==="future" ? "This date hasn't happened yet" : "No workout logged on this day"}
                    </div>
                  )}
                  ...
                  {dayPRs.length > 0 && (
                    <div style={{ marginTop:14, paddingTop:12, borderTop:`1px solid ${T.border}` }}>
                      <div style={{ fontSize:10, fontWeight:700, color:T.yellow, letterSpacing:"0.08em", marginBottom:8 }}>
                        🏆 PRs SET THIS DAY
                      </div>
                      {dayPRs.map(([name, pr]) => (
                        <div key={name} style={{ display:"flex", justifyContent:"space-between",
                          ...
                          <div style={{ display:"flex", gap:8 }}>
                            {pr.maxWeightDate===selectedDate && (
                              <span style={{ fontSize:11, fontWeight:700, color:T.yellow,
                                background:`${T.yellow}22`, padding:"2px 8px", borderRadius:20 }}>
                                {pr.maxWeight}kg max
                              </span>
                            )}
                            {pr.maxVolumeDate===selectedDate && (
                              <span style={{ fontSize:11, fontWeight:700, color:T.blue,
                                background:`${T.blue}22`, padding:"2px 8px", borderRadius:20 }}>
                                {pr.maxVolume}kg vol
                              </span>
                            )}
                          </div>
...
// ─── History + PRs (merged tab) ──────────────────────────────────────────────
function HistoryPRsView({ currentUser, onEditSession }) {
  const [inner, setInner] = useState("history");
  return (
    <div>
      {/* Inner segment toggle */}
      <div style={{ display:"flex", background:T.cardHi, borderRadius:12, padding:4,
        ...
        {[
          ...
          return (
            <motion.button key={s.id} onClick={()=>setInner(s.id)} whileTap={{scale:0.97}} style={{
              flex:1, background:active?T.blue:"none", border:"none", borderRadius:9,
              padding:"9px 0", cursor:"pointer", display:"flex", alignItems:"center",
              justifyContent:"center", gap:7,
              color:active?"#fff":T.muted, fontWeight:active?700:500, fontSize:13,
              boxShadow:active?`0 0 16px ${T.blue}44`:"none", transition:"all 0.18s"
            }}>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={inner} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}}
          exit={{opacity:0,y:-6}} transition={{duration:0.13}}>
          {inner==="history" ? <HistoryView currentUser={currentUser} onEditSession={onEditSession}/> : <PRsView currentUser={currentUser}/>}
        </motion.div>
      </AnimatePresence>
    </div>
  );
...
- Wellness tab: sleep tracker and protein tracker
...
export default function App() {
  ...
  const splitKey    = S.get(`wt_split_${currentUser.id}`,"ppl");
  const activeSplit = WORKOUT_SPLITS[splitKey] || WORKOUT_SPLITS.ppl;
