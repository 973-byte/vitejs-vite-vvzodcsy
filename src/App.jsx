  ...
  const p = {
    log: (
      <>
        ...
        <polyline points="17 11 9 11" />
        <polyline points="17 15 9 15" />
        <line x1="9" y1="7" x2="11" y2="7" />
        <path d="M18 2l2 2-9 9-3 1 1-3 9-9z" />
      </>
    ),
    progress: (
      <>
...
const totalVol = (sets) =>
  sets.reduce(
    (t, s) => t + (parseFloat(s.weight) || 0) * (parseInt(s.reps) || 0),
    0
  );
const fmtDate = (d) =>
...
function updatePRs(userId, exerciseName, sets) {
  ...
  let hit = false;
  if (mw > existing.maxWeight) {
    newPR.maxWeight = mw;
    newPR.maxWeightDate = todayISO();
    hit = true;
  }
  if (vol > existing.maxVolume) {
    newPR.maxVolume = vol;
    newPR.maxVolumeDate = todayISO();
    hit = true;
  }
  if (hit) {
    S.set(key, { ...prs, [exerciseName]: newPR });
  }
  return hit;
}
...
function getSuggestion(lastSets) {
  ...
  const reps =
    lastSets.find((s) => parseFloat(s.weight) === mw)?.reps ||
    lastSets[0]?.reps;
...
const mkCSS = (light) => `
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  body{background:${light ? '#F4F6FB' : T.bg};color:${
  light ? '#111827' : T.text
};font-family:-apple-system,'Inter',BlinkMacSystemFont,'Segoe UI',sans-serif;-webkit-font-smoothing:antialiased}
  input,button,textarea{font-family:inherit}
  input[type=number]::-webkit-inner-spin-button{-webkit-appearance:none}
  ::-webkit-scrollbar{width:4px;height:4px}
  ::-webkit-scrollbar-track{background:transparent}
  ::-webkit-scrollbar-thumb{background:${
    light ? '#CBD5E1' : T.border
  };border-radius:4px}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          style={{ position: 'absolute', marginTop: 28, textAlign: 'center' }}
        >
          <div
            style={{
              fontSize: 22,
...
function PlateCalc({ onClose }) {
  ...
  const possible =
    plates.reduce((t, p) => t + p.plate * p.count * 2, 0) + BAR_WEIGHT;
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 11, color: T.muted, marginTop: 6 }}>
          Bar: {BAR_WEIGHT}kg · Load per side shown
        </div>
        ...
        {target && (
          ...
          >
            {plates.length === 0 ? (
              <div
                style={{ color: T.muted, fontSize: 13, textAlign: 'center' }}
              >
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      fontWeight: 800,
                      color:
                        possible === parseFloat(target) ? T.green : T.yellow,
                    }}
                  >
                    {possible}kg{' '}
                    {possible !== parseFloat(target) && (
...
function PRBanner({ exercises, onDone }) {
  ...
  return (
    ...
    >
      ...
      <div>
        <div style={{ fontWeight: 800, fontSize: 13, color: '#0B0D1A' }}>
          🏆 New PR{exercises.length > 1 ? 's' : ''}!
        </div>
        <div style={{ fontSize: 11, color: '#0B0D1A88' }}>
          {exercises.join(', ')}
        </div>
      </div>
    </motion.div>
...
function ExerciseCard({
  ...
  return (
    <motion.div
      ...
      style={{
        background: T.card,
        border: `1px solid ${isPR ? T.yellow : T.border}`,
        borderRadius: 14,
        marginBottom: 12,
        overflow: 'hidden',
    ...
    >
      <div
        style={{
          height: 3,
          background: `linear-gradient(90deg,${accentColor},${T.yellow})`,
        }}
      />
          ...
          <div>
            ...
            {lastSession && (
              <div style={{ fontSize: 11, color: T.muted, marginTop: 2 }}>
                Last: {maxWeight(lastSession.sets)}kg —{' '}
                {lastSession.sets.map((s) => s.reps).join('/')} reps
              </div>
            )}
          </div>
          <motion.button
          ...
          >
            {[
              ...
              },
              {
                ...
                diff:
                  volDiff !== null
                    ? `${volDiff >= 0 ? '+' : ''}${Math.round(volDiff)}kg`
                    : null,
                up: volDiff >= 0,
              },
            ].map((stat) => (
              <div key={stat.label}>
                ...
                >
                  <span
                    style={{ fontSize: 16, fontWeight: 800, color: T.yellow }}
                  >
                    {stat.value}
                  </span>
                  {stat.diff && (
                    <span
                      style={{
                        fontSize: 11,
                        fontWeight: 600,
                        color: stat.up ? T.green : T.danger,
                      }}
                    >
                      {stat.diff}
                    </span>
    ...
    >
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          >
            Over<span style={{ color: T.yellow }}>load</span>
          </div>
        ...
        <Card style={{ padding: 24 }}>
          ...
          />
          <div style={{ fontSize: 13, color: T.muted, margin: '14px 0 6px' }}>
            Password
          </div>
          <Input
            type="password"
          ...
          />
          {error && (
            <div style={{ color: T.danger, fontSize: 12, marginTop: 10 }}>
              {error}
            </div>
          )}
        ...
        >
          Default: <span style={{ color: T.yellow }}>mukesh</span> /{' '}
          <span style={{ color: T.yellow }}>admin123</span>
        </div>
      </motion.div>
    </div>
  );
...
function AdminPanel({ currentUser }) {
  ...
  function addUser() {
    ...
    if (ex.find((u) => u.username === newUser.username)) {
  ...
  function deleteUser(id) {
    ...
    S.set(
      'wt_users',
      S.get('wt_users', []).filter((u) => u.id !== id)
    );
    S.set(
      'wt_sessions',
      S.get('wt_sessions', []).filter((s) => s.userId !== id)
    );
    refresh();
  }
  ...
  const statsPerUser = users.map((u) => ({
    ...u,
    sessions: allSessions.filter((s) => s.userId === u.id).length,
  }));
    ...
    <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div
                  style={{
                    ...
                    borderRadius: 10,
                    background:
                      u.role === 'admin' ? `${T.yellow}22` : `${T.blue}22`,
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                  }}
      ...
      <Card style={{ padding: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
          <Pill
            label="User"
            active={newUser.role === 'user'}
            onClick={() => setNewUser((p) => ({ ...p, role: 'user' }))}
          />
          <Pill
            label="Admin"
        ...
        {msg && (
          <div
            style={{
              color: msg.includes('!') ? T.green : T.danger,
              fontSize: 12,
              marginTop: 8,
            }}
          >
        ...
        <div style={{ marginTop: 14 }}>
          <GlowBtn full onClick={addUser}>
            <Icon name="plus" size={15} color="#fff" /> Create user
          </GlowBtn>
        </div>
      </Card>
            ...
            return (
              ...
              >
                ...
                >
                  <div>
                    <span
                      style={{ fontWeight: 600, fontSize: 13, color: m?.color }}
                    >
                      {m?.label}
                    </span>
                  ...
                  >
                    <Icon name="user" size={12} color={T.muted} />
                    <span style={{ fontSize: 11, color: T.muted }}>
                      {owner?.username || '?'}
                    </span>
                  </div>
...
function LogView({ currentUser, editingSession, onEditDone }) {
  ...
  const [day, setDay] = useState(() =>
    editingSession ? editingSession.day : todayName()
  );
  const [selected, setSelected] = useState(() =>
    editingSession ? editingSession.exercises.map((e) => e.name) : []
  );
  const [logData, setLogData] = useState(() =>
    editingSession
      ? Object.fromEntries(
          editingSession.exercises.map((e) => [
            e.name,
            e.sets.map((s) => ({ ...s })),
          ])
        )
  ...
  const lastSame = useMemo(() => {
    ...
    return sessions
      .filter(
        (s) =>
          s.day === day && s.date !== refDate && s.userId === currentUser.id
      )
  ...
  function cloneLastSession() {
    ...
    setLogData(
      Object.fromEntries(
        lastSame.exercises.map((e) => [e.name, e.sets.map((s) => ({ ...s }))])
      )
    );
    setShowPicker(false);
  }
  ...
  function toggleEx(ex) {
    ...
    if (!logData[ex])
      setLogData((d) => ({ ...d, [ex]: [{ weight: '', reps: '' }] }));
  }
  function handleSet(ex, i, f, v) {
    setLogData((d) => {
      const s = [...(d[ex] || [])];
      s[i] = { ...s[i], [f]: v };
      return { ...d, [ex]: s };
  ...
  function removeSet(ex, i) {
    setLogData((d) => {
      const s = d[ex].filter((_, j) => j !== i);
      return { ...d, [ex]: s.length ? s : [{ weight: '', reps: '' }] };
    });
  }
  function removeEx(ex) {
    setSelected((p) => p.filter((e) => e !== ex));
  }
    ...
    <div>
      <AnimatePresence>
        {prExercises.length > 0 && (
          <PRBanner exercises={prExercises} onDone={() => setPrExercises([])} />
        )}
      </AnimatePresence>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <motion.button
          ...
          style={{
            background: showTimer ? `${T.blue}22` : T.card,
            border: `1px solid ${showTimer ? T.blue : T.border}`,
            borderRadius: 10,
            padding: '8px 12px',
            cursor: 'pointer',
            display: 'flex',
        ...
        >
          <Icon name="timer" size={14} color={T.blue} />
        ...
        >
          <Icon name="plate" size={14} color={T.yellow} />
          <span style={{ fontSize: 12, fontWeight: 600 }}>Plates</span>
        </motion.button>
        {lastSame && (
          <motion.button
            whileTap={{ scale: 0.93 }}
            onClick={cloneLastSession}
            style={{
              background: T.card,
              border: `1px solid ${T.border}`,
              borderRadius: 10,
          ...
          >
            <Icon name="clone" size={14} color={T.green} />
            <span style={{ fontSize: 12, fontWeight: 600 }}>Clone last</span>
          </motion.button>
        )}
        <motion.button
          whileTap={{ scale: 0.93 }}
          ...
          style={{
            background: showNotes ? `${T.purple}22` : T.card,
            border: `1px solid ${showNotes ? T.purple : T.border}`,
            borderRadius: 10,
            padding: '8px 12px',
            cursor: 'pointer',
            display: 'flex',
        ...
        {showNotes && (
          ...
          >
            <textarea
              ...
              placeholder="Session notes... (sleep, energy, injuries, cues)"
              style={{
                width: '100%',
                background: T.cardHi,
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: T.muted }}>
                {fmtDate(editingSession.date)} · {editingSession.day}
              </div>
            </div>
          </div>
          <motion.button
        ...
        {DAYS.map((d) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: isEditing ? 1 : 0.94 }}
              style={{
                flexShrink: 0,
                background: act ? m.color : T.card,
                border: act ? 'none' : `1px solid ${T.border}`,
                borderRadius: 10,
                padding: '7px 14px',
                ...
                fontSize: 12,
                boxShadow: act ? `0 0 16px ${m.color}55` : 'none',
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        {lastSame && (
          <div style={{ fontSize: 11, color: T.muted, marginTop: 4 }}>
            Last {day}: {fmtDate(lastSame.date)}
          </div>
        )}
      </div>
      ...
      <motion.button
        ...
        style={{
          width: '100%',
          background: 'none',
          border: `1.5px dashed ${meta.color}55`,
          color: meta.color,
          borderRadius: 12,
          padding: '12px 0',
          fontSize: 13,
      ...
      {selected.length > 0 && (
        <GlowBtn
          full
          color={saved ? T.green : isEditing ? T.yellow : T.blue}
          onClick={saveSession}
        >
          <Icon
            name={saved ? 'check' : isEditing ? 'edit' : 'save'}
            size={16}
            color={isEditing && !saved ? T.bg : '#fff'}
          />
...
function ProgressView({ currentUser }) {
  ...
  const allSessions = S.get('wt_sessions', []).filter(
  ...
  const allExercises = useMemo(() => {
    const set = new Set();
    allSessions.forEach((s) => s.exercises.forEach((e) => set.add(e.name)));
    return [...set].sort();
  }, [allSessions.length]);

  useEffect(() => {
    if (!selEx && allExercises.length) setSelEx(allExercises[0]);
  }, [allExercises]);
    ...
    return allSessions
      ...
      .map((s) => {
        ...
        return {
          ...
          'Max weight': maxWeight(ex.sets),
          Volume: Math.round(totalVol(ex.sets)),
        };
      });
  }, [allSessions, selEx]);
  ...
  const bwChartData = useMemo(() => {
    return bwLog
      .slice(-12)
      .map((e) => ({ date: fmtShort(e.date), Bodyweight: e.weight }));
  }, [bwLog]);

  const totalSess = filtered.length;
  const totalVolAll = filtered.reduce(
    (t, s) => t + s.exercises.reduce((tt, e) => tt + totalVol(e.sets), 0),
    0
  );
    ...
    return (
      <div style={{ textAlign: 'center', paddingTop: 60 }}>
        ...
        <div style={{ color: T.muted, fontSize: 14 }}>
          Log your first session to see progress
        </div>
      </div>
    );
    ...
    <div>
      ...
      >
        {[
          ...
          { label: 'Volume', value: `${(totalVolAll / 1000).toFixed(1)}t` },
        ].map((k) => (
          <Card
            key={k.label}
            style={{ padding: '12px 10px', textAlign: 'center' }}
          >
            <div style={{ fontSize: 20, fontWeight: 800, color: T.yellow }}>
              {k.value}
            </div>
            <div
              style={{
      ...
      <Card style={{ marginBottom: 20 }}>
        ...
        >
          ...
          <Tag
            label={
              bwLog.length
                ? `${bwLog[bwLog.length - 1]?.weight}kg today`
                : 'Log today'
            }
            color={T.green}
          />
...
function HistoryView({ currentUser, onEditSession }) {
  ...
  useEffect(() => {
    setSessions(S.get('wt_sessions', []));
  }, []);

  const allSessions = sessions.filter((s) => s.userId === currentUser.id);
  const filtered = filterSessions(allSessions, timeFilter).sort(
    ...
    for (let w = 11; w >= 0; w--) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        days.push({
          date: iso,
          count: map[iso] || 0,
          day: DAYS[dt.getDay() === 0 ? 6 : dt.getDay() - 1].slice(0, 1),
        });
      }
      weeks.push(days);
    }
  ...
  function deleteSession(id) {
    ...
    S.set('wt_sessions', updated);
    setSessions(updated);
  }

  function exportCSV() {
    const rows = [
      ['Date', 'Day', 'Exercise', 'Set', 'Weight(kg)', 'Reps', 'Volume'],
    ];
    allSessions.forEach((s) =>
      s.exercises.forEach((e) =>
        e.sets.forEach((st, i) =>
    ...
    a.download = 'overload_sessions.csv';
    a.click();
  }

  const heatColors = (n) =>
    n === 0 ? T.border : n === 1 ? T.blueDim : n === 2 ? T.blue : T.yellow;
  ...
  return (
    <div>
              ...
              {week.map((d, di) => (
                <div
                  ...
                  style={{
                    ...
                    height: 14,
                    borderRadius: 3,
                    background: heatColors(Math.min(d.count, 3)),
      ...
      >
        ...
        >
          <Icon name="export" size={13} color={T.muted} />
        ...
        return (
          ...
          >
            <div
              style={{
                height: 3,
                background: `linear-gradient(90deg,${m.color},${T.yellow})`,
              }}
            />
            <div style={{ padding: '14px 16px' }}>
              <div
              ...
              >
                <div>
                  <span
                    style={{ fontWeight: 700, fontSize: 14, color: m.color }}
                  >
                    {m.label}
                  </span>
                ...
                >
                  <div
                    style={{ fontSize: 11, color: T.yellow, fontWeight: 700 }}
                  >
                    {Math.round(
                      s.exercises.reduce((t, e) => t + totalVol(e.sets), 0)
                    )}
                    kg vol
                  </div>
              ...
              {s.exercises.map((ex) => (
                ...
                >
                  <div
                    style={{ fontSize: 12, fontWeight: 600, marginBottom: 3 }}
                  >
                    {ex.name}
                  </div>
                  <div style={{ fontSize: 11, color: T.muted }}>
...
function PRsView({ currentUser }) {
  const prs = S.get(`wt_prs_${currentUser.id}`, {});
  const entries = Object.entries(prs).sort(
    (a, b) => b[1].maxWeight - a[1].maxWeight
  );
  ...
  if (!entries.length)
    return (
      <div style={{ textAlign: 'center', paddingTop: 60 }}>
        ...
        />
        <div style={{ fontSize: 16, fontWeight: 700, color: T.muted }}>
          No PRs yet
        </div>
        <div style={{ fontSize: 13, color: T.muted, marginTop: 6 }}>
          Save sessions to start tracking records
        </div>
      </div>
    );
      ...
      {entries.map(([name, pr], i) => (
        ...
        >
          <div
            style={{
              height: 3,
              background: `linear-gradient(90deg,${T.yellow},#FF9500)`,
            }}
          />
          <div style={{ padding: '14px 16px' }}>
            <div
...
function WellnessView({ currentUser }) {
  ...
  const [timerActive, setTimerActive] = useState(
    () => !!S.get(`wt_sleep_timer_${uid}`, null)
  );
  ...
  function deleteSleep(id) {
    const u = sleepLog.filter((e) => e.id !== id);
    S.set(`wt_sleep_${uid}`, u);
    setSleepLog(u);
  }
  ...
  const avgSleep = filteredSleep.length
    ? Math.round(
        filteredSleep.reduce((t, e) => t + e.durationMins, 0) /
          filteredSleep.length
      )
    : 0;
  const avgQuality = filteredSleep.length
    ? (
        filteredSleep.reduce((t, e) => t + e.quality, 0) / filteredSleep.length
      ).toFixed(1)
    : 0;
  const bestSleep = filteredSleep.length
    ? Math.max(...filteredSleep.map((e) => e.durationMins))
    : 0;
  ...
  const [proteinTarget, setProteinTarget] = useState(() =>
    S.get(`wt_ptarget_${uid}`, 160)
  );
  const [proteinLog, setProteinLog] = useState(() =>
    S.get(`wt_protein_${uid}`, [])
  );
  ...
  const todayTotal = todayProtein.reduce(
    (t, m) => t + (parseFloat(m.grams) || 0),
    0
  );
  ...
  function saveTarget() {
    const t = parseInt(targetInput) || 160;
    setProteinTarget(t);
    S.set(`wt_ptarget_${uid}`, t);
    setEditTarget(false);
  }
  function addMeal() {
    if (!mealGrams) return;
  ...
  function deleteMeal(id) {
    const u = proteinLog.filter((m) => m.id !== id);
    S.set(`wt_protein_${uid}`, u);
    setProteinLog(u);
  }

  // filtered protein records — group by date
  const filteredProtein = filterByPeriod(proteinLog, proteinFilter);
  const filteredDates = [...new Set(filteredProtein.map((m) => m.date))]
    .sort()
    .reverse();
  const totalProteinPeriod = filteredProtein.reduce(
    (t, m) => t + (parseFloat(m.grams) || 0),
    0
  );
  const daysHitTarget = filteredDates.filter((date) => {
    const tot = filteredProtein
      .filter((m) => m.date === date)
      .reduce((t, m) => t + (parseFloat(m.grams) || 0), 0);
    return tot >= proteinTarget;
  }).length;
  const avgProtein = filteredDates.length
    ? Math.round(
        filteredProtein.reduce((t, m) => t + (parseFloat(m.grams) || 0), 0) /
  ...
  return (
    <div>
      ...
      <SectionHeader
        ...
        color={T.blue}
        title="Sleep Tracker"
        sub="Timer or manual · quality rating"
      />
        ...
        {timerActive ? (
          <div style={{ textAlign: 'center', padding: '8px 0' }}>
            ...
            <div style={{ fontSize: 12, color: T.muted, margin: '8px 0 20px' }}>
              Started ·{' '}
              {new Date(timerStart).toLocaleTimeString('en-IN', {
                hour: '2-digit',
                minute: '2-digit',
              })}
            </div>
          ...
          <>
              ...
              {[1, 2, 3, 4, 5].map((q) => (
                <motion.button
                  ...
                  style={{
                    flex: 1,
                    background:
                      quality === q ? qualityColors[q] + '33' : T.cardHi,
                    border: `1.5px solid ${
                      quality === q ? qualityColors[q] : T.border
                    }`,
                    borderRadius: 8,
                    padding: '7px 0',
                    cursor: 'pointer',
                    color: quality === q ? qualityColors[q] : T.muted,
                    fontSize: 10,
                    fontWeight: 700,
                  }}
                >
            ...
            <GlowBtn
              full
              color={sleepSaved ? T.green : T.blue}
              onClick={saveSleepManual}
            >
              <Icon
                name={sleepSaved ? 'check' : 'bed'}
                size={15}
                color="#fff"
              />
        ...
        <>
          ...
          >
            {[
              {
                label: 'AVG SLEEP',
                value: fmtDuration(avgSleep),
                color: T.blue,
              },
              {
                label: 'BEST NIGHT',
                value: fmtDuration(bestSleep),
                color: T.yellow,
              },
              {
                label: 'AVG QUALITY',
                value: `${avgQuality}/5`,
                color: qualityColors[Math.round(Number(avgQuality))] || T.green,
              },
            ].map((k) => (
              <Card
                key={k.label}
              ...
              >
                <div style={{ fontSize: 13, fontWeight: 800, color: k.color }}>
                  {k.value}
                </div>
                <div
                  style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div
                    style={{
                      ...
                      height: 40,
                      borderRadius: 12,
                      background: `${qualityColors[entry.quality]}22`,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      flexShrink: 0,
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: T.muted }}>
                      {fmtDate(entry.date)} · {entry.savedAt}
                    </div>
                    {entry.note && (
                      <div
                        style={{
      ...
      >
        <SectionHeader
          ...
          sub={`Daily target: ${proteinTarget}g protein`}
        />
        <motion.button
          whileTap={{ scale: 0.9 }}
          onClick={() => setEditTarget((p) => !p)}
          style={{
            ...
            <Card style={{ padding: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <Input
                  ...
                  placeholder="160"
                />
                <GlowBtn color={T.green} small onClick={saveTarget}>
      ...
      <Card style={{ marginBottom: 14 }}>
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            >
              {Math.round(todayTotal)}g
            </div>
            <div style={{ fontSize: 12, color: T.muted, marginBottom: 8 }}>
              {pct >= 1
                ? 'Target hit!'
                : `${Math.round(proteinTarget - todayTotal)}g to go`}
            </div>
            <div
              style={{
                height: 6,
            ...
            >
              <motion.div
                ...
                style={{
                  ...
                  background:
                    pct >= 1
                      ? `linear-gradient(90deg,${T.green},${T.yellow})`
                      : `linear-gradient(90deg,${T.green},${T.blueHi})`,
                }}
              />
            </div>
          </div>
          ...
          >
            ...
            {todayProtein.map((meal, i) => (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span
                    style={{ fontSize: 16, fontWeight: 800, color: T.green }}
                  >
                    {meal.grams}g
                  </span>
                  <motion.button
      ...
      {filteredDates.length === 0 ? (
        ...
        >
          ...
          />
          <div style={{ color: T.muted, fontSize: 13 }}>
            No protein records in this period
          </div>
        </Card>
      ) : (
        <>
          ...
          >
            {[
              ...
              {
                label: 'TOTAL',
                value: `${Math.round(totalProteinPeriod)}g`,
                color: T.blue,
              },
              {
                label: 'DAYS ON TARGET',
                value: `${daysHitTarget}/${filteredDates.length}`,
                color: T.yellow,
              },
            ].map((k) => (
              <Card
          ...
          <Card style={{ marginBottom: 24 }}>
            ...
            >
              DAILY BREAKDOWN · {filteredDates.length}{' '}
              {proteinFilter === 'Daily'
                ? 'day'
                : proteinFilter === 'Weekly'
                ? 'days this week'
                : 'days this month'}
            </div>
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 8,
                          borderRadius: '50%',
                          background: hit ? T.green : T.muted,
                        }}
                      />
...
export default function App() {
  ...
  function handleLogin(user) {
    setCurrentUser(user);
    S.set('wt_session', user);
  }
  function handleLogout() {
    setCurrentUser(null);
    S.set('wt_session', null);
  ...
  const userSessions = S.get('wt_sessions', []).filter(
    (s) => s.userId === currentUser.id
  );
          ...
          >
              ...
              >
                ...
                {streak > 0 && (
                  ...
                  >
                    ...
                    <span
                      style={{ fontSize: 11, fontWeight: 700, color: T.orange }}
                    >
                      {streak}d
                    </span>
                  </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                >
                  <Icon
                    name={currentUser.role === 'admin' ? 'crown' : 'user'}
                    size={14}
